...
def build_solver_settings(precision: type[np.floating[Any]]) -> Dict[str, Any]:
    ...
    defaults: Dict[str, Any] = {
        ...
        "min_step_shrink": precision(0.2),
        "max_step_growth": precision(2.0),
...
def build_step_controller_settings(
    ...
    base_settings: Dict[str, Any] = {
        ...
        "order": 5,
        "min_step_shrink": precision(solver_settings["min_step_shrink"]),
        "max_step_growth": precision(solver_settings["max_step_growth"]),
...
def create_controller(
    ...
    controller = CPUAdaptiveController(
        ...
        precision=precision,
        min_step_shrink=settings["min_step_shrink"],
        max_step_growth=settings["max_step_growth"],
...
def run_reference_loop_with_history(
    ...
    *,
    ...
    if driver_coefficients is not None:
        drivers_fn = driver_evaluator.with_coefficients(
            np.asarray(driver_coefficients, dtype=precision)
        )
    else:
        drivers_fn = driver_evaluator
    ...
    drivers_initial = drivers_fn(precision(t))
    ...
    while t < end_time - equality_breaker:
        ...
        try:
            sampled = drivers_fn(precision(t))
