...
def validate_request(path, expected_sha):
    ...
    if earlier is not None and (
        earlier["model_sha256"] != request["model_sha256"]
...
def prepare(args):
    ...
    for path in (SCRIPT, model.SCRIPT):
    ...
    if args.reuse_predictions is not None:
        ...
        if (
            earlier["model_sha256"] != model.digest(model.SCRIPT)
            or earlier["hardware"] != hardware
        ):
    ...
    for name in CASE_NAMES:
        ...
        if origin is not None:
            ...
            cases[name] = old
            print(json.dumps(dict(reused=name, predictions=8)), flush=True)
            continue
        for block in (32, 64):
            for mode in ("promote", "addressable"):
                for contract in (False, True):
                    ...
                    plan["provenance"]["input_graph"] = file_record(graph_path)
                    plan["provenance"]["hardware_manifest"] = file_record(
                        args.hardware,
                    )
                    path = directory / f"b{block}_{mode}_{contract}.json"
...
from benchmarks.hardware_model import native_plan as model
from benchmarks.hardware_model import native_plan_holdout as observer
