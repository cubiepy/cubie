"""Explicit native worker for a reviewed same-cell store/load composite."""
...
spec = importlib.util.spec_from_file_location(
    "store_composite_controller", HERE / "benchmark_source.py"
)
...
def main():
    ...
    try:
        ...
        module = control.load_module(
            HERE / "kernel.py", "store_composite_kernel"
        )
        ...
        target = f"st.{REQUEST['space']}.u32 [write_address], x;"
        ...
        expected_shared = 4096 if REQUEST["space"] == "shared" else 0
        if resources["static_shared_bytes"] != expected_shared:
            raise ValueError("Composite static shared frame differs")
        if (
            REQUEST["space"] == "shared"
            and resources["local_bytes_per_thread"]
        ):
            raise ValueError("Shared composite has an unexpected local frame")
        if (
            REQUEST["space"] == "local"
            and resources["local_bytes_per_thread"] < 4
        ):
            raise ValueError("Local composite lost its addressable cell")
        ...
        if REQUEST["mode"] == "compile_only":
            ...
            return
        result["native_certificate"] = control.native_certificate(
            HERE, result, REQUEST
        )
        prior = None
        ...
        def launch(
            ...
            expected = control.expected_bits(
                REQUEST["operation"],
                seeds,
            ...
            control.validate_memory(trace_values, expected, seeds, warps)
