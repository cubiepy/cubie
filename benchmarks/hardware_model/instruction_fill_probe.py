"""Prepare and measure register-only instruction streams and victim paths."""
...
AGGRESSOR:
""" + fmas(body_ffmas) + """
    setp.ne.u32 priming, warm, 0;
    @priming bra WARM_ENTRY;
TIMED_ENTRY:
    mov.u64 begin, %clock64;
    mov.u32 phase, 1;
...
def parameters(args):
    ...
    if args.victim_blocks not in (1, 2, 4, 8):
...
def prepare(args):
    ...
    write(root / "source_specification.json", dict(
        **spec, generator_sha256=sha(__file__), ptx_sha256=sha(ptx),
        ...
        timed_scope=("stream loop plus endpoint/control administration"
                     ...
                     "PCs; aggressor, warm traversal and output stores excluded"),
    ...
    commands = []
    for command in (
        [str(assembler), "-arch=sm_89", "-O3", "-v", str(ptx), "-o",
...
def main():
    ...
    parser.add_argument("--kind", choices=("stream", "victim"), default="stream")
    ...
    parser.add_argument("--populations", default="1,4,8,16,32")
