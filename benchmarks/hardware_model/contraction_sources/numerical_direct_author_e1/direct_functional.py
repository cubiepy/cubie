...
def run(manifest_path, output):
    ...
    prior = read(checked(manifest["prior_receipt"]))
    ...
    workload = next(
        x for x in frozen["workloads"] if x["id"] == "workload_006"
    )
    ...
    try:
        ...
        for source in ("0002", "0000"):
            ...
            retained = next(
                ...
                if x["case"] == case_id and x["mode"] == "contract_false"
            )
            source_record = next(
        ...
        for fma in (True, False):
            receipt["comparisons"].append(
                dict(
                    final_fma=fma,
                    **compare_arrays(
                        saved[("0002", fma)]["state"],
                        saved[("0000", fma)]["state"],
                        tolerance,
                    ),
                )
            )
