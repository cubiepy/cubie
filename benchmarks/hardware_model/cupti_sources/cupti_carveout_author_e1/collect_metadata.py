...
def run(percent, output_root):
    ...
    try:
        build_path = ROOT / "build_receipt.json"
        ...
        with ExitStack() as stack:
            for directory in (CUPTI.parent, CUDA / "bin", ROOT):
                stack.enter_context(os.add_dll_directory(str(directory)))
            vendor = ctypes.WinDLL(str(CUPTI))
            collector = ctypes.CDLL(str(ROOT / "collector.dll"))
            paths = [
            ...
            if [os.path.normcase(str(path.resolve())) for path in paths] != [
                ...
                for path in (CUPTI, ROOT / "collector.dll")
            ]:
                raise ValueError(
        ...
        receipt["status"] = "ACTIVITY_METADATA_AND_EXACT_E5_OUTPUT_PASS"
    except Exception:
