...
| `IntegratorRunSettings.py` | `IntegratorRunSettings(CUDAFactoryConfig)`: `algorithm`, `step_controller`, `precision` and the captured `loop_fn`. |
...
The run derives nothing; the Solver passes resolved settings. Order:
1. `OutputFunctions` from `output_settings` and the system's sizes.
2. `IntegratorRunSettings` from `algorithm` and `step_controller` (default `fixed`).
3. The step from `algorithm_settings`, the system's sizes and device functions, and
   `is_adaptive` (`step_controller != "fixed"`); an explicit step on a mass-matrix
   system raises.
4. The controller from `step_control_settings`, `n_states`, `mass_flags` and the step's
   `algorithm_order`.
5. `DAEInitialiser` from the step's `settings_dict`, `algorithm_settings` and the sizes.
6. `IVPLoop` from `loop_settings` and the children's products; the step, controller and
   initialiser (`aliases="algorithm_shared"`) register under it; `loop_fn` is captured.

`update()` runs the system, the outputs, this run's names, then each child with the same
dict plus the products it consumes; a new `algorithm` or `step_controller` swaps that
child, built from the update and the old compile flags. `build()` returns `loop_fn`.
`settings_dict` merges the children's; `grouped_settings()` splits it into the
