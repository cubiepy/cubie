...
changed; (2) applies `_apply_performance_defaults` (under `auto_performance`, an implicit
step's `performance_defaults` plus `unroll_newton_exits` rolled when the unrolled
instruction count overflows the instruction cache, skipping user-given keys); (3)
