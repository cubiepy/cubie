...
`updates_dict`. Controller gains (`CONTROLLER_GAIN_PARAMETERS`) do not carry across a controller change; the new controller uses its own gain defaults unless the update supplies them. Gains or `filter_coefficients` without a `step_controller` (at construction or in `update()`) promote an `i`/`pi`/`pid` in effect to the smallest chain member carrying them via `_resolve_controller_name`/`_promote_controller`; `gustafsson`/`fixed` are never promoted, and a named controller drops gains it lacks.
