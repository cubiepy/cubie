...
- Keys in `ALL_ALGORITHM_STEP_PARAMETERS` are step defaults (`step_default_settings`); every other key is a controller default (`controller_default_settings`). The `family_defaults(tableau)` classmethod and `algorithm_facts(algorithm, tableau)` (`AlgorithmFacts`: step class, tableau, defaults, `has_error_estimate`, `is_implicit`, `is_linear`) give the table without a step instance; the Solver applies it.
