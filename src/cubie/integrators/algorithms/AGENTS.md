...
- `BaseAlgorithmStep.algorithm_defaults` is the merged table as an `AlgorithmDefaults`; `ALL_ALGORITHM_STEP_PARAMETERS` keys are step defaults (`step_default_settings`), the rest controller defaults (`controller_default_settings`). The run resolves and writes `RUN_RESOLVED_STEP_PARAMETERS`, the kernel `KERNEL_RESOLVED_STEP_PARAMETERS`; both sit in the step's `injected_keys`.
...
tableau's `stage_coefficients` to its norm), then runs `wire_helpers()`
