...
DEFAULT_DT_MIN = 1e-6
"""Minimum step when neither ``dt_min`` nor ``dt`` is given."""

DEFAULT_DT_MAX = 1.0
"""Maximum step when neither ``dt_max`` nor ``dt`` is given."""
...
class AdaptiveStepControlConfig(BaseStepControllerConfig):
    ...
    def __attrs_post_init__(self) -> None:
        """Reject given step bounds that contradict each other."""
        ...
        if dt is not None and dt_min is not None and dt < dt_min:
            raise ValueError(
                f"dt ({dt}) < dt_min ({dt_min}). "
                f"Provide a compatible dt or adjust dt_min."
            )
        if dt is not None and dt_max is not None and dt > dt_max:
            raise ValueError(
    ...
    def dt_min(self) -> float:
        """Given, else ``dt/100``, else the default under a given dt_max."""
        if self._dt_min is not None:
            return self.precision(self._dt_min)
        if self._dt is not None:
            return self.precision(self._dt / 100)
        dt_min = DEFAULT_DT_MIN
        if self._dt_max is not None and self._dt_max < dt_min:
            dt_min = self._dt_max / 100
        return self.precision(dt_min)

    @property
    def dt_max(self) -> float:
        """Given, else ``dt*100``, else the default over a given dt_min."""
        if self._dt_max is not None:
            return self.precision(self._dt_max)
        if self._dt is not None:
            return self.precision(self._dt * 100)
        dt_max = DEFAULT_DT_MAX
        if self._dt_min is not None and self._dt_min > dt_max:
            dt_max = self._dt_min * 100
        return self.precision(dt_max)

    @property
    def dt(self) -> float:
        """Given, else the geometric mean of the bounds."""
        if self._dt is not None:
            return self.precision(self._dt)
        return self.precision(sqrt(float(self.dt_min) * float(self.dt_max)))
