...
| `base_step_controller.py` | `BaseStepController` / `BaseStepControllerConfig` / `ControllerCache`; `ALL_STEP_CONTROLLER_PARAMETERS` (union of every controller's kwargs); `CONTROLLER_GAIN_NAMES` (PID gain keys) and `CONTROLLER_GAIN_PARAMETERS` (gain keys plus `filter_coefficients`, excluded from swap carryover); `BaseStepController.gain_names` (gain keys the config class carries); `GAIN_CONTROLLER_CHAIN` + `minimal_gain_controller` + `promoted_gain_controller` (smallest `i`/`pi`/`pid` for a set of nonzero gains); `FILTER_COEFFICIENT_PRESETS` + `filter_coefficients_to_gains` (`(beta1,beta2,beta3)` → gains); `mass_flags` (one per state, default all differential). |
...
- **Step bounds are plain fields**: adaptive `dt_min` (1e-6), `dt_max` (1.0) and `dt`
  (the bounds' geometric mean when unset); fixed `dt` (1e-3). Contradictory bounds
  raise `ValueError`. Deriving bounds from a lone `dt` is the Solver's
  (`solver_settings.resolve_step_bounds`).
- `ControllerCache` carries `is_adaptive`, `dt`, `dt_min`, `dt_max`, `atol`, `rtol` as
  product fields.
...
- **Gains** come from the Solver: family defaults for the family's controller, given gains otherwise; a swapped-in controller is built from the update's keys.
