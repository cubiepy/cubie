...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel, runs=)` through a `ComparisonRunner` and applies the best `LaunchResult` through `apply_launch`. `auto_size=True` sizes the batch to `waves` times `most_resident_runs` over every candidate and probes the duration to `target_ms`; `auto_size=False` times the grid as passed and warns under two waves. `performance_defaults(given, step, system)` returns the built step's placement and unroll settings under `auto_performance`, never a given one. |
