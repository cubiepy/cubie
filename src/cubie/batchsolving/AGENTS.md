...
| File | Description |
|------|-------------|
| `solver.py` | `Solver` + `solve_ivp()` — the public API. `solve_ivp` also accepts raw equations (callable / string / iterable of strings), building the system via `_system_from_equations` (state names from a `y0` dict, parameter defaults from a `parameters` dict; array `parameters` rejected). `Solver` owns `system_interface`, `input_handler` (`BatchInputHandler`), and `kernel` (`BatchSolverKernel`); `driver_interpolator` is a passthrough to the kernel-owned interpolator, and most getters are thin pass-throughs to `kernel`. `Solver.compile()` runs the same input processing as `solve` and compiles the kernel without launching. `Solver.given` (a `SolverSettings`) and `Solver.effective` (an `EffectiveSettings`) are the settings the user set and the settings in use; `Solver.is_given(name)`; `Solver.settings_dict` is `given.as_kwargs()`; `Solver.copy()` is `Solver(system.copy(), **settings_dict)` at the current logging level, with the parent's drivers (`kernel.driver_inputs()`) configured; `optimisation_candidates(force)` keeps the given performance keys fixed. |
