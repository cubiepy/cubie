...
| File | Description |
|------|-------------|
| `solver.py` | `Solver` + `solve_ivp()` — the public API. `solve_ivp` also accepts raw equations (callable / string / iterable of strings), building the system via `_system_from_equations` (state names from a `y0` dict, parameter defaults from a `parameters` dict; array `parameters` rejected). `Solver` owns `system_interface`, `input_handler` (`BatchInputHandler`), and `kernel` (`BatchSolverKernel`); `driver_interpolator` is a passthrough to the kernel-owned interpolator, and most getters are thin pass-throughs to `kernel`. `Solver.compile(drivers=None, duration=..., settling_time=..., t0=..., **kwargs)` applies the settings and drivers as `solve` does and compiles the kernel without preparing a batch (batch inputs play no part in compilation). `Solver.given` (a `SolverSettings`) and `Solver.effective` (an `EffectiveSettings`) are the settings the user set and the settings in use; `Solver.is_given(name)`; `Solver.settings_dict` is `given.as_kwargs()`; `Solver.copy()` is `Solver(system.copy(), **settings_dict)` at the current logging level, with the parent's drivers (`kernel.driver_inputs()`) configured; `optimisation_candidates(force)` keeps the given performance keys fixed. |
| `solver_settings.py` | `SolverSettings`, one field per setting a Solver accepts (`None` = not given) on `_CubieConfigBase`, and `EffectiveSettings` (adds the names only resolution sets; its `as_kwargs` keeps a `None` interval or memory proportion). `clashing_names` lets `BaseODE` reject a constant named like a setting. |
...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel)` on one compiled `parent.copy()` per candidate and applies the best `LaunchResult` through `apply_launch`. `auto_size=True` sizes the batch (`sized_batch_runs`) and duration; `auto_size=False` times the grid as passed. Uncached candidates compile in spawn workers; timing uses kernel events under the `"silent"` logger level. `performance_defaults(given, step, system)` returns the built step's placement and unroll settings under `auto_performance`, never a given one. |
