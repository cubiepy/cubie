...
| File | Description |
|------|-------------|
| `solver.py` | `Solver` + `solve_ivp()` — the public API. `solve_ivp` also accepts raw equations (callable / string / iterable of strings), building the system via `_system_from_equations` (state names from a `y0` dict, parameter defaults from a `parameters` dict; array `parameters` rejected). `Solver` owns `system_interface`, `input_handler` (`BatchInputHandler`), and `kernel` (`BatchSolverKernel`); `driver_interpolator` is a passthrough to the kernel-owned interpolator, and most getters are thin pass-throughs to `kernel`. `Solver.compile(drivers=None, duration=..., settling_time=..., t0=..., **kwargs)` applies settings and drivers and compiles the kernel; no batch is allocated. `Solver.given` (a `SolverSettings`) and `Solver.effective` (an `EffectiveSettings`) are the settings the user set and the settings in use; `Solver.is_given(name)`; `Solver.settings_dict` is the given record over the children's retained values (a child value neither given nor resolved that differs from the child's declared default); `Solver.copy()` is `Solver(system.copy(), **settings_dict)` at the current logging level, with the parent's drivers (`kernel.driver_inputs()`) configured; `optimisation_candidates(force)` keeps the given performance keys fixed. |
...
- **Launch geometry:** `launch_geometry(blocksize=None, runs=None)` returns a launch's block
  ...
  L2 rule under `auto_performance`). `runs` is the launch's run count; `None` sizes a full block.
  `launchable_shapes(blocksizes, runs=None)` lists the block sizes that launch whole with
  their dynamic shared bytes and natural block counts. Neither needs a batch; the run path
  passes its chunk's run count. The launch rules
