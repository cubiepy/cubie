...
| `location_tuning.py` | `Solver.tune_locations` / `auto_memory="tune"` backend: `tune_locations` compiles one shared variant per candidate buffer in worker processes (`python -m cubie.batchsolving.location_tuning`, pickled job on stdin), times the variants whose compiled local frame shrank with interleaved solves, then pairs of the winners, applies the best and persists it under `<cache root>/location_tuning/`. |
