...
A driverless kernel's layout has a zero first dimension. Each solve passes the
interpolator's `coefficients` to `InputArrays`, which uploads a table object once.
