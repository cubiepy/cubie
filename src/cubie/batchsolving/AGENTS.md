...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the core's `optimisation_candidates(force)` at `launch_candidates(kernel)` on one compiled `parent.copy()` per candidate and applies the best `LaunchResult` through `apply_launch`. `mode="auto"` sizes the batch (`sized_batch_runs`) and duration; `mode="given"` times the grid as passed. Uncached candidates compile in spawn workers; timing uses kernel events under the `"silent"` logger level. |
