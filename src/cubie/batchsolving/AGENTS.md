...
- **Shared-memory sizing:** shared memory is a statically sized `cuda.shared.array` baked into
  the compiled kernel from the `blocksize` compile setting (`BatchSolverConfig.blocksize`; the
  kernel also carries `launch_bounds` for it, and changing `blocksize` recompiles).
  `limit_blocksize` halves the block size at build time until the block's shared footprint fits
  under a 32 KiB ceiling; `shared_memory_needs_padding` adds a 4-byte skew only for single
