...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel, runs=)` through a `ComparisonRunner` and applies the best `LaunchResult` through `apply_launch`. `auto_size=True`: batch = `tail_safe_runs` at `waves` of the most resident launch; duration ramped toward `target_ms`, never past the given; then the batch grows toward `BatchSolverKernel.single_chunk_runs` if short, shrinks if the first probe overshoots `OVERSHOOT_FACTOR`, and is corrected once past `CORRECTION_FACTOR`. `auto_size=False` times the grid as passed and warns under two waves. Both warm the clocks first; `_timing_plan` sets solves per round (`LONG_LAUNCH_MS`) and the lead-in (`LEAD_IN_MS`). `performance_defaults(given, step, system)` returns the built step's placement and unroll settings under `auto_performance`, never a given one. |
...
- The timing protocol and the success-tier ranking live in `comparison.py`; `optimize` sets solves per round and lead-in from the sized launch (`_timing_plan`), `calibrate` uses the defaults.
