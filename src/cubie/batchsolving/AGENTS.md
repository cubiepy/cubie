...
- **Kept across solves:** the system snapshot identity (`system_config_stale`), the chunk
  ...
  on (rebuilt on a chunk-count or verbosity change; a run queued before the kernel's
  `synchronize` takes its own event set, and `recent_kernel_ms(count)` reads the last
  `count` runs' kernel times).
...
- The timing protocol and the success-tier ranking are fixed in `comparison.py` and shared; neither entry point takes protocol parameters.
