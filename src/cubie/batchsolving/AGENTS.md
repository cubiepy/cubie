...
| `solver_settings.py` | `SolverSettings`: one attrs field per user-facing setting (`None` = not provided), generated from the children's `ALL_*` keyword sets plus the Solver's own (`tableau`, `system_constants`, `duration`, `save_variables`, `summarise_variables`); `EffectiveSettings` adds `DERIVED_NAMES`. `from_kwargs` (grouped dicts flatten; unknown or renamed names raise), `updated` (`None` unsets), `as_kwargs` (provided values), `as_updates` (the provided values the kernel takes, loose keys). `resolve(given, system, interface)` -> `Resolution` (effective record, notices): algorithm facts, controller name and gains, step bounds, family/tableau/DAE step defaults, inner tolerances, output selection (labels to indices), loop schedule for `duration`. `resolve_performance(given, effective, step, system, previous)`: the step's placement defaults and the instruction-cache rule for Newton unrolling; `auto_performance` off keeps `previous`. `settings_differ` compares two records by value. |
...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel)` on `parent.copy()` and applies the best `LaunchResult` through `apply_launch`. Spawn workers pre-warm the kernel cache; timing uses kernel events under the `"silent"` logger level. |
...
### Solver: settings
`__init__` and `update` record `given`, write `precision`, `operation_ordering` and
`system_constants` into the system, resolve `effective`, push changed values down
`kernel.update`, then `resolve_performance` on the built step and push its changes.
Notices warn once. Child `update` calls are `silent=True`. `duration` is a setting only the
...
- **Launch geometry:** `launch_geometry(blocksize=None)` returns a launch's block size and
  ...
  64); `run(blocksize=None)` uses it; `Solver.blocksize_given` says whether it was provided. `shared_memory_needs_padding` adds a 4-byte skew only
