...
| File | Description |
|------|-------------|
| `solver.py` | `Solver` + `solve_ivp()` — the public API. `solve_ivp` also accepts raw equations (callable / string / iterable of strings), building the system via `_system_from_equations` (state names from a `y0` dict, parameter defaults from a `parameters` dict; array `parameters` rejected). `Solver` owns `system_interface`, `input_handler` (`BatchInputHandler`), and `kernel` (`BatchSolverKernel`); `driver_interpolator` is a passthrough to the kernel-owned interpolator, and most getters are thin pass-throughs to `kernel`. `Solver.compile(drivers=None, duration=..., settling_time=..., t0=..., **kwargs)` applies the settings and drivers as `solve` does and compiles the kernel without preparing a batch (batch inputs play no part in compilation). `Solver.given` (a `SolverSettings`) and `Solver.effective` (an `EffectiveSettings`) are the settings the user set and the settings in use; `Solver.is_given(name)`; `Solver.settings_dict` is the given record over the children's retained values (a child value neither given nor resolved that differs from the child's declared default); `Solver.copy()` is `Solver(system.copy(), **settings_dict)` at the current logging level, with the parent's drivers (`kernel.driver_inputs()`) configured; `optimisation_candidates(force)` keeps the given performance keys fixed. |
...
| `optimize.py` | `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel, blocksizes, runs)` on one compiled `parent.copy(mem_proportion=None)` per candidate (twins join the automatic memory pool; a failed build closes the twins made so far) and applies the best `LaunchResult` through `apply_launch`. Device grids are copied to the host once. `auto_size=True` sizes the batch to `waves` times `most_resident_runs` over every candidate's launches and scales the duration; `auto_size=False` times the grid as passed. The fewest achieved waves over all launches is recorded and warned about once. Uncached candidates compile in spawn workers; timing uses kernel events under the `"silent"` logger level. `performance_defaults(given, step, system)` returns the built step's placement and unroll settings under `auto_performance`, never a given one. |
...
- **Launch geometry:** `launch_geometry(blocksize=None, runs=None)` returns a launch's block
  size and dynamic shared bytes: `limit_blocksize` halves the block size until its shared
  footprint fits the per-block limit, then a pad holds `resident_blocks` per SM (`None` = the
  L2 rule under `auto_performance`). `runs` bounds the dynamic shared bytes of a launch
  smaller than a block; `None` sizes a full block. `launchable_shapes(blocksizes, runs=None)`
  lists the block sizes that launch whole with their dynamic shared bytes and natural block
  counts. Both size on the compiled specialization (typed on the unit stand-in arrays), so
  they need no batch: the run path passes its chunk's run count, and a caller sizing before
  a solve passes the batch it intends. The launch rules
  ...
  `default_launches` per `runs` (`None` = a full block), plus `duration_counts`, `output_array_heights`,
