...
| File | Description |
|------|-------------|
| `solver.py` | `Solver` + `solve_ivp()` — the public API. `solve_ivp` also accepts raw equations (callable / string / iterable of strings), building the system via `_system_from_equations` (state names from a `y0` dict, parameter defaults from a `parameters` dict; array `parameters` rejected). `Solver` owns `system_interface`, `input_handler` (`BatchInputHandler`), and `kernel` (`BatchSolverKernel`); `driver_interpolator` is a passthrough to the kernel-owned interpolator, and most getters are thin pass-throughs to `kernel`. `Solver.compile()` runs the same input processing as `solve` and compiles the kernel without launching. `Solver.settings` is a `SolverSettings` (`user_given` and `effective`); `__init__` and `update` resolve `effective` through `solver_settings.resolve_settings`, push it to the kernel, then `performance_defaults` from the built step; `solve`/`compile` re-resolve a duration-derived summary window. `Solver.settings_dict` is `user_given` with the output selection and the logger level; `Solver.copy()` is `Solver(system.copy(), **settings_dict)` (drivers not copied); `optimisation_candidates(force)` drops the given performance keys; `blocksize_given`. |
| `solver_settings.py` | `SolverSettings` (`give` records updates, `None` unsets, the last of a filter or loose gains wins); `resolve_settings(user_given, system, duration)` -> `ResolvedSettings`: family/tableau/DAE step defaults, controller name (family default promoted to carry given gains, `fixed` when the step has no error estimate) and gains, `resolve_step_bounds`, `resolve_inner_tolerances`, `resolve_loop_timing` (schedule and `save_last`/`save_regularly`/`summarise_regularly`; an unset window takes the duration). |
| `BatchSolverKernel.py` | `BatchSolverKernel(CUDAFactory)` — the batch `@cuda.jit` kernel; maps each run to the `SingleIntegratorRun` device loop. Owns the `ArrayInterpolator` as a direct child factory (`driver_interpolator`; `configure_drivers()` updates it and the dependent compile settings as one unit; `run()` raises `ValueError` when the system declares drivers and no evaluator is wired). `build_kernel()` attaches a `CUBIECache` built from the config's `cache` settings and `config_hash` to the dispatcher and keeps it for the flush-on-change path in `_invalidate_cache`. Defines `RunParams` (frozen: duration/warmup/t0/runs + chunk metadata) and `BatchSolverCache`; owns the `InputArrays`/`OutputArrays` managers and memory-manager registration. `compile()` allocates the batch and compiles the specialised kernel through the disk cache without launching, via the backend-selected `compile_kernel_specialization` in `backend/utils`. `settings_dict` is the kernel's `ALL_KERNEL_PARAMETERS` fields, `lineinfo`, the plain memory keys and the core's `settings_dict`; `copy()` rebuilds on `system.copy()` sharing the memory manager. |
...
| `optimize.py` | `performance_defaults(user_given, step, system)`: `unroll_newton_exits` rolls when the unrolled step overflows the instruction cache, a FIRK `stage_increment_location` is shared above `SHARED_STAGE_INCREMENT_MIN_STATES`; `PERFORMANCE_PARAMETERS`. `Solver.optimize` backend: `run_optimization` times the solver's `optimisation_candidates(force)` at `launch_candidates(kernel)` on `parent.copy()` and applies the best `LaunchResult` through `apply_launch`. Spawn workers pre-warm the kernel cache; timing uses kernel events under the `"silent"` logger level. |
...
`ALL_ALGORITHM_STEP_PARAMETERS`, `ALL_LOOP_SETTINGS`, `ALL_KERNEL_PARAMETERS`); a kwarg no
...
- **Launch geometry:** `launch_geometry(blocksize=None)` returns a launch's block size and
  ...
  64); `run(blocksize=None)` uses it; `Solver.blocksize_given` says whether it was given. `shared_memory_needs_padding` adds a 4-byte skew only
