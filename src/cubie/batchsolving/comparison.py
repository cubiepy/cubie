...
def _compile_candidate(
    system_bytes: bytes, settings: Dict[str, Any], cache_root: Any
) -> str:
    """Compile one settings set in a worker; return its error, if any."""
    ...
    solver = None
    try:
        solver = Solver(pickle.loads(system_bytes), **settings)
...
class ComparisonRunner:
    ...
    inits, params
        The ``(variable, run)`` grid to time on; ``None`` compiles only.
    ...
    def open(self) -> None:
        ...
        self._record = solver.settings_dict(for_new_process=True)
        self._touched = set()
    ...
    def close(self) -> None:
        ...
        try:
            solver = self._solver
            solver.update(
                {key: self._given.get(key) for key in self._touched},
                silent=True,
            )
    ...
    def _reject(self, candidate: Candidate, error: str) -> None:
        """Record ``candidate`` as rejected with ``error``."""
    ...
    def compile(self, candidates: Sequence[Candidate]) -> List[Candidate]:
        ...
        for candidate in candidates:
            ...
            try:
                ...
                # Invalid parameters from grids like calibration's raise.
                self._reject(candidate, f"{type(exc).__name__}: {exc}")
                continue
        ...
        if missing:
            self.select(missing.pop(0))
            solver.kernel.compile()
            compile_seconds = default_timelogger.get_event_duration(
                "compile_cuda_kernel"
            )
        if missing and worth_parallelising(
    ...
    def _compile_in_pool(self, candidates: Sequence[Candidate]) -> None:
        ...
        system_bytes = pickle.dumps(self._solver.system)
        cache_root = get_cache_root_override()
        payloads = [
            (system_bytes, {**self._record, **candidate.settings}, cache_root)
            for candidate in candidates
        ]
        context = multiprocessing.get_context("spawn")
        with context.Pool(min(self._max_parallel, len(payloads))) as pool:
            errors = pool.starmap(_compile_candidate, payloads)
        for candidate, error in zip(candidates, errors):
            if error:
                self._reject(candidate, error)
            else:
                self.emit(f"  {candidate.label}: worker compiled")
