...
class BatchSolverKernel(CUDAFactory):
    ...
    def __init__(
        ...
        unroll_settings = dict(unroll_settings or {})
        kernel_settings = dict(kernel_settings or {})
        unroll = UnrollFlags(**unroll_settings)
        ...
        if kernel_settings:
            self.update_compile_settings(kernel_settings, silent=True)
        if lineinfo is not None:
            self.update({"lineinfo": lineinfo}, silent=True)
    ...
    def update(
        ...
        **kwargs
        ...
        Notes
        -----
        Order: interpolator, run, this kernel's settings with the run's
        ``loop_fn`` and compile flags.
        """
    ...
    def copy(self) -> "BatchSolverKernel":
        ...
        return type(self)(
            ...
            unroll_settings={
                key: getattr(unroll, key) for key in ALL_UNROLL_PARAMETERS
            },
            memory_settings={
            ...
            **self.single_integrator.grouped_settings(),
        )
