...
- `unroll_if(range(n), flag[, count])` loops resolve in `_mlir_compat._UnrollIfPass` from the closure `flag` (bool or `(unroll, count)` pair; an explicit `count` wins while unrolling): `True` → `cuda.unroll(range(n))`, `(True, k>1)` → `cuda.unroll(range(n), k)`, `(True, 1)` → `cuda.nounroll(range(n))`, `False` → plain `range(n)` (backend decides). Hints bind as `_cubie_unroll`/`_cubie_nounroll` globals via `TransformContext.stored_values`; body `consteval(...)` reading the loop variable is stripped; a wheel without the hints raises at transform time. Identity fallback in `cuda_simsafe.unroll_if`.
