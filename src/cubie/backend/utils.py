...
WARP_SCHEDULERS_PER_MULTIPROCESSOR = 4
"""Warp schedulers (sub-partitions) per SM on every supported device."""
...
def minimum_scheduler_blocks(hardware: DeviceHardware, blocksize: int) -> int:
    """Return the resident blocks that give every warp scheduler a warp."""
    warps = WARP_SCHEDULERS_PER_MULTIPROCESSOR * hardware.warp_size
    return max(1, -(-warps // blocksize))
...
__all__ = [
    ...
    "WARP_SCHEDULERS_PER_MULTIPROCESSOR",
    "DeviceHardware",
    "KernelResources",
