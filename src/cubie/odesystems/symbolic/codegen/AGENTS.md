...
| `lu_solver.py` | Emits the direct sparse LU solves for `LUSolver`: `generate_lu_solve_code(..., variant)` returning `(code, lu_nnz)`: Markowitz-ordered symbolic factorisation of `W = beta*M - gamma*a_ij*h*J` emitted as straight-line elimination and substitution with literal indices. `beta`/`gamma` (and `a_ij` when the step's diagonals are uniform) fold in as numeric literals; pivots floor sign-preservingly via `copysign(max(|d|, floor), d)` and the solve always returns `int32(0)`. Diagonal factor slots hold reciprocal (real) or inverse (complex) pivots, so eliminations and substitutions multiply. Prefactored `cached_aux` layouts are compacted by `_prefactored_slot_plan`, shared by the prepare, solve, and smoothing generators: structurally zero entries read as literal zero and duplicate entries alias one slot. |
