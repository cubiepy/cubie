...
| `preconditioners.py` | Emits the Neumann-series and diagonal-Jacobi preconditioners: `generate_neumann_preconditioner_code(..., variant)` and `generate_jacobi_preconditioner_code(..., variant)`, each covering all four variants through one template. Emitted signature ends `..., v, out, jvp`. Both families run a truncated series and consume `order`: Neumann expands about beta*I (factory default 1), Jacobi about the operator's own diagonal (factory default 0 = the plain diagonal solve). `order` is a factory binding, never a source input: a Jacobi source carries the diagonal-only body and the series body side by side and the factory picks one when bound. Series terms recover the removed `h_eff*J_ii` as `beta*M_ii - D_i` from the diagonal already computed, and divide by `_cubie_codegen_safe_diag_*` from outside the assignment list, so `_jacobi_output_symbols` passes those names to `prune_unused` as outputs. |
