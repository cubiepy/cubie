...
| `fused_operators.py` | Emits the fused operator-preconditioner helpers (`generate_fused_operator_code` + at-state/cached/n-stage variants): one device function computing `z_out = P(v)` and `out = A(z_out)` from a single math graph. The preconditioner member sequence (jacobi/neumann kinds) and the operator merge *before* CSE from raw Jacobian entries (shared `_cubie_codegen_j_*` locals), so Jacobian subexpressions compute once, `z` lives in scalar locals instead of a scratch array, and the ordering policy sees the whole per-iteration DAG. Neumann members unroll to `preconditioner_order` (Horner), which therefore enters the source identity; the cached variant reads the `prepare_jac` slot layout unchanged. Device signature: `(state, parameters, drivers, [cached_aux,] base_state, t, h, a_ij, v, z_out, out)`. |
