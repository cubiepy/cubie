...
| `linear_operators.py` | Emits the matrix-free linear operator and JVP cache helpers: `generate_operator_apply_code`, `generate_operator_apply_at_state_code` (error smoothing; no increment substitution), `generate_cached_operator_apply_code`, `generate_prepare_jac_code` (populates `cached_aux`, returns `(code, aux_count)`), `generate_cached_jvp_code`, `generate_n_stage_linear_operator_code` (flattened FIRK), `generate_mass_apply_code` (`mass_apply(v, out)` = `M @ v`). `*_from_jvp` variants take a prebuilt `JVPEquations`. |
