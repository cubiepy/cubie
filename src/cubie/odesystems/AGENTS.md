...
| `solver_helpers.py` | Solver-helper request/product containers and request identity: `SolverHelperKind`, `HELPER_KIND_TRAITS` (the single authority for kind-level traits — stage awareness, chained membership; `STAGE_AWARE_KINDS`/`CHAINED_KINDS`/`FUSED_KINDS` are derived views), frozen `SolverHelperRequest` (kind, beta, gamma, order, canonical stage spec, `chained_kinds` for composed preconditioners), `HelperResult` (device callable + `cached_auxiliary_count`), mutable `SolverHelperCache` (`factories[source_hash]`, `members[member_hash]`). |
