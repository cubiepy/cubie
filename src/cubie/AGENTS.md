...
- **Cache products.** A cache field declared `product_field()` is filled by `_build`
  from the same-named factory property; parents read children's `products` at update
