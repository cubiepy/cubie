...
| `_utils.py` | Shared helpers: `PrecisionDType`, precision/buffer validators + converters, attrs validator factories, `device_function_field`, `build_config`, `merge_kwargs_into_settings`, `ensure_nonzero_size`, `slice_variable_dimension`, `clamp_factory`. |
...
- **Settings resolve at the `Solver`**, which pushes one flat effective dict down
  `update`; a factory derives nothing from whether a value was provided.
  `build_config` folds loose nested keys (`unroll_*`, `lineinfo`, `cache_*`) as
  `update` does.
