...
- **Compile-time loops:** `for i in unroll_if(range(n), flag)` with `flag` a closure local read from `compile_settings.unroll`; warp-voted iteration loops take `unroll_converged_exits`; runtime-bounded loops use plain `range`.
