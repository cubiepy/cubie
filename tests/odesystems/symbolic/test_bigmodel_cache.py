"""Unit tests for BigModel caching functionality."""
...
@pytest.fixture
def basic_bigmodel_path(bigmodel_fixtures_dir):
    """Return path to basic_ode.bigmodel fixture."""
    return str(bigmodel_fixtures_dir / "basic_ode.bigmodel")


@pytest.fixture
def tmp_bigmodel_file(tmp_path):
    """Create temporary BigModel file for testing."""
    bigmodel_content = """<?xml version="1.0" encoding="UTF-8"?>
<model xmlns="http://www.bigmodel.org/bigmodel/1.0#">
    <component name="test">
        <variable name="x" initial_value="1.0"/>
    </component>
</model>
"""
    tmp_file = tmp_path / "test.bigmodel"
    tmp_file.write_text(bigmodel_content)
    return str(tmp_file)


def test_cache_initialization_valid_inputs(basic_bigmodel_path):
    """Verify BigModelCache initializes with valid model_name and bigmodel_path.
    ...
    """
    cache = BigModelCache(model_name="basic_ode", bigmodel_path=basic_bigmodel_path)
    
    # Verify attributes are set correctly
    assert cache.model_name == "basic_ode"
    assert cache.bigmodel_path == basic_bigmodel_path
    assert "basic_ode" in str(cache.cache_dir)


def test_cache_initialization_invalid_inputs(basic_bigmodel_path):
    ...
    with pytest.raises(TypeError, match="model_name must be str"):
        BigModelCache(model_name=123, bigmodel_path=basic_bigmodel_path)
    
    # Test non-string bigmodel_path
    with pytest.raises(TypeError, match="bigmodel_path must be str"):
        BigModelCache(model_name="test", bigmodel_path=Path(basic_bigmodel_path))
    
    # Test empty model_name
    with pytest.raises(ValueError, match="model_name cannot be empty"):
        BigModelCache(model_name="", bigmodel_path=basic_bigmodel_path)
    
    # Test non-existent bigmodel_path
    with pytest.raises(FileNotFoundError, match="BigModel file not found"):
        BigModelCache(model_name="test", bigmodel_path="/nonexistent/file.bigmodel")


def test_get_bigmodel_hash_consistent(tmp_bigmodel_file):
    ...
    """
    cache = BigModelCache(model_name="test", bigmodel_path=tmp_bigmodel_file)
    
    # Get hash twice - should be identical
    hash1 = cache.get_bigmodel_hash()
    hash2 = cache.get_bigmodel_hash()
    assert hash1 == hash2
    ...
    with open(tmp_bigmodel_file, 'a') as f:
        f.write("\n<!-- Modified -->\n")
    
    hash3 = cache.get_bigmodel_hash()
    assert hash3 != hash1


def test_serialize_args_consistent(tmp_bigmodel_file):
...
def test_compute_cache_key_different_args(tmp_bigmodel_file):
...
def test_compute_cache_key_includes_edited_values(tmp_bigmodel_file):
...
def test_cache_valid_missing_file(basic_bigmodel_path, isolated_cache_root):
...
def test_cache_valid_hash_mismatch(tmp_bigmodel_file, isolated_cache_root):
    """Verify cache_valid() returns False when file hash doesn't match.
    
    Tests cache invalidation when the source BigModel file has been
...
def test_load_from_cache_returns_none_invalid(
    tmp_bigmodel_file, isolated_cache_root
):
...
def test_save_and_load_roundtrip(tmp_bigmodel_file, isolated_cache_root):
    ...
    # Verify expected keys are present (note: no 'bigmodel_hash' in new format)
...
def test_corrupted_cache_returns_none(tmp_bigmodel_file, isolated_cache_root):
    ...
    actual_hash = cache.get_bigmodel_hash()
...
def test_lru_eviction_on_sixth_entry(tmp_bigmodel_file, isolated_cache_root):
...
def test_cache_hit_with_different_params(tmp_bigmodel_file, isolated_cache_root):
...
def test_file_hash_change_invalidates_all_configs(
    ...
    Tests that when the source BigModel file is modified, all cached
...
@pytest.fixture
def isolated_cache(basic_bigmodel_path, tmp_path):
    """Return a BigModelCache whose storage is redirected to tmp_path."""
    cache = BigModelCache(
        model_name="basic_ode", bigmodel_path=basic_bigmodel_path
    )
